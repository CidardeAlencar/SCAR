...
class EditedwScreen extends StatefulWidget {
  final userID;
  //EditeduScreen({Key? key, required this.user}) : super(key: key);
  EditedwScreen({required this.userID});
  //static const route = '/';

  @override
  State<EditedwScreen> createState() => _EditedwScreenState();
}

class _EditedwScreenState extends State<EditedwScreen> {
  late User user;
    ...
    try {
      ...
      if (snapshot.exists) {
        setState(() {
          ...
          numFusilController.text = data['numFusil'];
          // numCerrojoController.text = data['numCerrojo'];
          // numCierreController.text = data['numCierre'];
          // novedadesController.text = data['novedades'];
          responsableController.text = data['responsable'];
          _selectedOption = data['estado'];
          _selectedOption2 = data['patrulla'];
          _selectedOption3 = data['numCerrojo'];
          _selectedOption4 = data['numCierre'];
          _selectedOption5 = data['novedades'];
        });
      } else {
  ...
  void initState() {
    super.initState();
    // Llama a una función para obtener los datos del usuario desde Firestore
    obtenerDatosUsuario(widget.userID);
  }
  ...
  void signUp() async {
    ...
    try {
      ...
      DocumentReference weaponDocRef =
          FirebaseFirestore.instance.collection('weapons').doc(widget.userID);
      await weaponDocRef.update({
        // 'email': email,
        'numFusil': fusil,
      ...
      final snackBar = SnackBar(content: Text('Datos editados correctamente.'));
